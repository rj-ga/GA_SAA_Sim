...
class TestCompanionComputer(CompanionComputer):
    ...
    def init(self):
        ...
        t2 = threading.Thread(target=self.lidar.update_sitl_sensor)#/*/*
        ...
        while True:
            pass
        #ADD ALL THE CODE TO TEST HERE
            #self.compute_simple_stop_near_obstacle_waypoint()
            #self.Testing_Velocity_Input_Movement()
    ...
    def handle_mission_current(self,msg,nextwaypoint):
        ...
        try:
            if msg.seq > nextwaypoint:
                ...
                p_point_index = int(msg.seq)-1
                if p_point_index < 0: #Handle 1st waypoint
                    p_point_index = 0
                print(c_point_index,p_point_index)
                print ("Moving to waypoint ",c_point_index,self.waypoints[c_point_index])
                self.current_waypoint = self.waypoints[c_point_index]
                self.previous_waypoint = self.waypoints[p_point_index]
                self.i_current_waypoint = self.i_waypoints[c_point_index]
                self.i_previous_waypoint = self.i_waypoints[p_point_index]
            ...
            if(msg!=None and msg.get_type()!=None):
                try:
                    ...
                    if msg.get_type() == 'MISSION_CURRENT':
                        ...
                        #print ('Next Waypoint',self.next_waypoint,"Current Waypoint",self.current_waypoint,'Previous Waypoint',self.previous_waypoint)
                        #print (('Inertial_Next Waypoint', self.i_next_waypoint,"Current Waypoint",self.i_current_waypoint,'Previous Waypoint',self.i_previous_waypoint))
    ...
    def Testing_Velocity_Input_Movement(self,x,y,z): #This is just for me to obtain testing waypoints
        #x,y,z = 1,0,0 #Change the value of x to make the drone move north, y to make it move east and z for along height, specify these as velocities
        if(self.currentMode=='GUIDED'):
            self.mavlinkInterface.mavConnection.mav.send(mavutil.mavlink.MAVLink_set_position_target_local_ned_message( 10, self.mavlinkInterface.mavConnection.target_system, self.mavlinkInterface.mavConnection.target_component, mavutil.mavlink.MAV_FRAME_LOCAL_NED, int(0b110111000111), 0, 0, 0, x, y, z, 0, 0, 0, 0, 0))
        else:
            override = 1
            print("Not in GUIDED, Pilot Probably Overrided")

    def compute_simple_stop_near_obstacle_waypoint(self):
        if self.currentMode == 'AUTO':
            if(self.sensor_trig==1):#Triggers if the sensor sees a val at 20m away
                self.brakeMode()
                print("Breaked!")
                Way_Point_X = self.i_current_waypoint[0]
                Way_Point_Y = self.i_current_waypoint[1]
                Vx = Way_Point_X - self.i_previous_waypoint[0]
                Vy = Way_Point_Y - self.i_previous_waypoint[1]
                Mag = math.sqrt(Vx*Vx + Vy *Vy)
                Vx = Vx/Mag #Unit Vector Along X and Y
                Vy = Vy/Mag
                self.GuidedMode()
                print("Guided Mode")
                self.Testing_Velocity_Input_Movement(Vx,Vy,0)
    ...
    def check_sensor_Raw(self):  #Triggers avoidance if the front portion of the sensor detects obstacle under 5m
        print("Started Front Sensor Triggerer")
        while True:
            try:
                #/*/*add heading direction
                #print(self.lidar.raw_data[15:50])
                d = self.lidar.raw_data[20:50]
                for i in d:
                    if i<20:
                        print("Triggered",i)
                        self.sensor_trig=1
