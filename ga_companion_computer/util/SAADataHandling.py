        ...
        if len(data)<=10:
            ...
            for i in range(len(data)):
                ...
                if sensor_binary_mask[i] == True:
                    ...
                    self.count+=1
                    if(self.X.size==0):
                        self.X=np.array([float(data[i]*math.cos(self.delta_angle*i + self.init_angle))])
                        self.Y=np.array([float(data[i]*math.sin(self.delta_angle*i + self.init_angle))])
                        
                        
                    else:
                        self.X=np.concatenate((self.X,np.array([float(data[i]*math.cos(self.delta_angle*i + self.init_angle))])),axis=0)
                        self.Y=np.concatenate((self.Y,np.array([float(data[i]*math.sin(self.delta_angle*i + self.init_angle))])),axis=0)
