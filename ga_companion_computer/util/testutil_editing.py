...
class TestCompanionComputer(CompanionComputer):
    ...
    def init(self):
        ...
        self.world_map = [(10,-2),(8,-16),(18,-27),(36,-36),(-1,-5),(-12,-8),(-8,2),(-6,-25),(-19,-17),(-30,-25),(-16,-35),(-22,-7),(2,-41),(3,-21),(0,-2),[0,-21]]
    ...
    def Obtain_Waypoints(self):
        ...
        for i in range(0,self.waypoint_count,1):
            ...
            self.i_waypoints.append((local_coord[1],-local_coord[0]))
    ...
    def DijKstras_Triggerer(self,world_map): #This needs to be called everytime. Need to set a trigger such that I can call it after it has completed its execution Part
        ...
        for c in local_obstacle_list:# Checks if any obstacle in that grid is in the viscinity of 2x2 from the drone
            if((abs(c[0]-self.px)<=2 and abs(c[1]-self.py)<=2) or self.sensor_trig==1):
                if(self.currentMode == 'AUTO'):
                    ...
                    self.Dijktras_Manueaver(local_obstacle_list,int(gx),int(gy))
    ...
    def check_sensor_Raw(self):  #Triggers avoidance if the front portion of the sensor detects obstacle under 3m
        while True:
            try:
                for i in range(15,45,1):
                    if self.lidar.raw_data[20:45]<5:
                        print("Triggered Avoidance")
                        self.sensor_trig=1
                        time.sleep(0.02)
                        break
                    else:
                        print("No Obstacle Detected Around")
                        self.sensor_trig=0
            except:
                pass
    ...
    def DijKtras_Map_Goal_Generation(self,local_obstacle_list,l,b):#Finds the goal coordinate where the drone must go to in that grid using Vectors
        ...
        Way_Point_X = self.i_current_waypoint[0]######Need to Compute this portion
        ...
        for i in range(10,40,1): #Tries to Place the goal coordinate about 6m away in the heading direction.
            flag = 1
    ...
    def Dijktras_Manueaver(self,obs,goal_x,goal_y):#Pass obstacles x and  y near that proximity
        ox = []
        oy = []
        ...
        c = self.dijktras_PathPlanning(ox, oy, grid_size, robot_radius,sx, sy, gx, gy)
        c = c[::-1]
