...
import threading
import logging
#NON-SIM-CODE#import util.PrasRPLidar as prasLidar
# import util.nano_radar_can5 as nanoRadar
#NON-SIM-CODE##import util.Ti_UART as tiRadar
...
class TestCompanionComputer(CompanionComputer):
    def __init__(self, sitlType, sitlPort):
        ...
        #NON-SIM-CODE#self.lidar = prasLidar.PrasRPLidar_(4,-90,2,10)#scanMode, OffsetAngle, MinRange and MaxRange
        ...
        print('init')
        #Front sensor
        #NON-SIM-CODE#self.front_sensor = estimation.Sensor(1,1*math.pi/180,10,2,0)
        #SITL
...
#            print("Rel: ", self.relativeAlt)
 ...
 #This function retrieves the GPS Coordinates of the drone at prese
...
#whenever in auto reset all the markers so that planning can be started again.
    ...
    def update_vars(self):
        ...
        temp_x=self.nanoRadar.final_x
        temp_y=self.nanoRadar.final_y
    
        temp__x=[]
        temp__y=[]

        for i in range(0,min(len(temp_x),len(temp_y))):
            
            if(temp_x[i]<40 and abs(temp_y[i])<10):
                temp__x.append(round(temp_x[i],2))
                temp__y.append(round(temp_y[i],2))
        
        #self.front_sensor
        #print(self.front_sensor.data)
        temp_x=temp__x
        temp_y=temp__y
       # print('temp__x',temp__x)

        if(len(temp_x) != 0 and len(temp_y) != 0):
            if (np.array(temp_x)).max() < 40 and (np.array(temp_y)).max() < 40:
    ...
    def log_data(self):
        ...
        final_data = "Time," + str(time.time())+'{}' + str(self.log_time1) + '{}' + roll_pitch_yaw_string + terrain_alt_str + selfpx_and_selfpy_and_selfpz +vx_vy_vz+number+'{}' + frontSensorX +'{}' + frontSensorY +'{}' +'{}' + str(self.braked) + '{}' + self.currentMode + '\n'
...
##################General realtime data handling ##############################
        ...
        #print("sizex", (self.front_sensor.X).shape)
        print("y: ", self.front_sensor.Y)
        #print("sizey", (self.front_sensor.Y).shape)
