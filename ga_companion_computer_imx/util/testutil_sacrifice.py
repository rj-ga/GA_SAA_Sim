...
#assumes 50 m field on either side
    ...
    def init(self):
        ...
        #self.scheduledTaskList.append(ScheduleTask(0.005,self.front_sensor.handle_raw_data))  # 0.05
        ...
        #self.check_sensor_Raw()
             ...
             #self.DijKstras_Triggerer(np.array(self.mat_list))
    ...
    def obstacle_in_direction(self,direc_vec,global_map,des_angle,des_distance):
        ...
        for i in range(np.size(global_map,axis=0)):
            ...
            if((obstacle_vector_magnitude>=0.5 and obstacle_vector_magnitude<=des_distance)):                    
                ...
                if abs(obstacle_angle)<des_angle and obstacle_angle>0:
    ...
    def basic_guided_switch_and_plan(self):
        if self.relativeAlt>1: #todo add current waypoint before to handle edge cases
           
            # bool_temp,self.dist_temp=self.obstacle_in_direction(self.navigation_controller.pos_vector,self.np_mat_to_list,self.desired_ang,self.engaging_distance)
            # self.dist_safe=self.dist_temp-self.safety_margin
            
            # if(bool_temp and self.marker==0): #direction vector and angle
                #todo add waypoint vector above

            self.wp_list=np.array([[0,0],[0,-300],[4,-300],[4,0],[8,0],[8,-300],[12,-300],[12,0]])

           
            #self.i_current_waypoint=[0,-225]
            #self.i_previous_waypoint=[0,0]
            ...
            #switching logic if in a straight line?
            if(math.pow((self.px-self.i_current_waypoint[0]),2)+math.pow((self.py-self.i_current_waypoint[1]),2)<625):
                self.desired_ang=120
            else:
                self.desired_ang=10

            if(self.currentMode=='AUTO'):
                #    self.wpvec=[self.i_current_waypoint[0]-self.i_previous_waypoint[0],self.i_current_waypoint[1]-self.i_previous_waypoint[1]]
                ...
                temp_wp_dist_vec=[self.i_current_waypoint[0]-self.px,self.i_current_waypoint[1]-self.py] #vector towards next waypoint
                



                if(bool_temp and self.vec.mag2d(temp_wp_dist_vec)>abs(self.dist_temp) ): #if obstacle is not near boudnry and in front of waypoint then avoid
                                      
                    self.dist_safe=self.dist_temp-self.safety_margin #heuristic for safety (5m as of now)
                    ...
                    self.GuidedMode() #keeps sending mavlink command till not switched to guided only if in auto mode
                    print('hey switched to guided--',self.guided_target[0],'---',{self.guided_target[1]})
            ...
            if(self.currentMode=='GUIDED' and self.changed_from_auto_to_guided==1):
                if(abs(self.px-self.guided_target[0])+abs(self.py-self.guided_target[1])<4 and abs(self.vx)+abs(self.vy)<1 and self.reached_A==False):
                    self.reached_A=True
                    print('reached A')

            if(self.reached_A):
                if(self.obstacle_near_ninety_degree()==3):

#self.checked is used to ensure that if you have checkked an obstacle is in front then you replan and keep moving
                    if(not(self.reached_B) and not (self.reaching_B)and self.checked==1):
                        dist=5
                    ...
                    if(abs(self.px-self.guided_target[0])+abs(self.py-self.guided_target[1])<4 and self.reaching_B):
                        bool_temp,self.dist_temp=self.obstacle_in_direction([self.wpvec[0],self.wpvec[1]],self.np_mat_to_list,self.desired_ang,10)
                        self.checked=1
                        if(not(bool_temp)):
                            self.reached_B=True
                    ...
                    if(abs(self.px-self.guided_target[0])+abs(self.py-self.guided_target[1])<1 and self.reaching_C):
                        bool_temp,self.dist_temp=self.obstacle_in_direction([self.wpvec[1],self.wpvec[0]],self.np_mat_to_list,self.desired_ang,10)
                        self.checked=1
                    ...
                    if(self.reached_C==True and not self.reaching_D):
                    ...
                    if(abs(self.px-self.guided_target[0])+abs(self.py-self.guided_target[1])<1 and self.reaching_D):
                        print('reached D')
                        self.AutoMode()
                ...
                elif(self.obstacle_near_ninety_degree()==2): #obstacle while moving in short direction
                    pass

                elif(self.obstacle_near_ninety_degree()==1): #obstacle while moving in longer direction
                    ...
                    if(self.reached_A and not(self.reached_B) and not (self.reaching_B)and self.direction_decided==1):
...
#whenever in auto reset all the markers so that planning can be started again.
        self.reached_A=self.reached_B=self.reached_C=self.reaching_C=self.reaching_B=self.reaching_D=False
        self.changed_from_auto_to_guided=0
        self.checked=1
    ...
    def update_mission(self):
        # print(' current', self.i_current_waypoint)
        # print('previous',self.i_previous_waypoint)
        # print('next',self.i_next_waypoint)
        #self.update_vars()  
        #print(self.c_point_index,'wp')
        #self.jump_wayPoint(5)
       
        # self.add_new_message_to_sending_queue(mavutil.mavlink.MAVLink_command_long_send(self.mavlinkInterface.mavConnection.target_system,
        #                                                                                        self.mavlinkInterface.mavConnection.target_component,
        #                                                                                        mavutil.mavlink.MAV_CMD_DO_JUMP, # command
        #                                                                                        0,                                       # confirmation
        #                                                                                        5,                                       # param1 
