...
class TestCompanionComputer(CompanionComputer):
    def __init__(self, sitlType, sitlPort):
        ...
        self.tiRadar = tiRadar.TiRadar("/home/pi/IMX_Sense_Stop_Radar/ga_companion_computer/util/config_for_flashing.cfg")
...
# #   takes in direction vector and a map in global coordinates to give minimum distance to obstacle in that sector, returns true if obstacle is present                        
    ...
    def get_mission_coordinates(self):#Gets the whole mission accurately, including takeoff and land if present
        try:
            ...
            self.homeLocation = self.i_waypoints[0]  #Assuming Home position is the takeoff Position     
...
#whenever in auto reset all the markers so that planning can be started again.
    ...
    def log_data(self):
        ...
        roll_pitch_yaw_string = str(self.roll)+','+str(self.pitch)+','+str(self.yaw)+"{}"
        terrain_alt_str = str(self.terrainAlt)+"{}"
        selfpx_and_selfpy_and_selfpz = str(self.px)+','+str(self.py)+','+str(self.pz)+'{}'
        ...
        frontSensorX = ','.join([str(elem) for elem in self.front_sensor.X]) + '{}'
        frontSensorY = ','.join([str(elem) for elem in self.front_sensor.Y])+"{}"
        frontSensorZ = ','.join([str(elem) for elem in self.front_sensor.Z])+'{}'
        vx_vy_vz = str(self.vx)+','+str(self.vy)+'{}'
        radvel = str(self.front_sensor.vel)+'{}'
        number=str(self.front_sensor.X.shape[0])
        final_data = "Time," + str(time.time()-self.log_time)+'{}' + roll_pitch_yaw_string + terrain_alt_str + selfpx_and_selfpy_and_selfpz + gps_lat_lon_of_drone + frontSensorX + frontSensorY + frontSensorZ+ vx_vy_vz+radvel+number+'\n'
        #final_data = frontSensorX + frontSensorY + frontSensorZ+ vx_vy_vz+radvel+'\n'
        
        self.csv_file.write(final_data)
        if time.time()-self.log_time>=1000:
            self.csv_file.close()
