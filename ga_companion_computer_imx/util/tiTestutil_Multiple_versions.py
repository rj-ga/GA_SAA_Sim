...
#whenever in auto reset all the markers so that planning can be started again.
        ...
        if(time.time()-self.tiRadar._read_time<0.3):
            if(len(temp_x) != 0 and len(temp_y) != 0 and len(temp_z) != 0 and len(temp_vel) != 0):
                if (np.array(temp_x)).max() < 40 and (np.array(temp_y)).max() < 40 and (np.array(temp_z)).max() < 40 and (np.array(temp_vel)).max() < 10:
                        ...
                        self.front_sensor.Z = -1*np.array(temp_z)
    ...
    def create_csv_file(self):
        ...
        self.csv_file = open(str(self.log_time1)+'PRAS.csv','w')
    ...
    def log_data(self):
        ...
        final_data = "Time," + str(time.time())+'{}' + roll_pitch_yaw_string + terrain_alt_str + selfpx_and_selfpy_and_selfpz +vx_vy_vz+number+'{}' + frontSensorX +'{}' + frontSensorY +'{}' + frontSensorZ +'{}' + radvel+  '\n'
        #final_data = frontSensorX + frontSensorY + frontSensorZ+ vx_vy_vz+radvel+'\n'
...
#####################This is for realtime sensing of data#############################
            ...
            if(self.currentMode=='LOITER' and self.terrainAlt>2):#and self.i_current_waypoint!=0 and self.terrainAlt>2 and self.i_current_waypoint!="TakeOff"):
                #if something is in heading direction, within desired angle and engaging distance, then engage brake/switch to guided
                #print('inside loop')
