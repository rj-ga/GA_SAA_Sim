...
#import util.nano_radar_can5 as nanoRadar
import util.Ti_UART as tiRadar
...
# filename = './main_log'+str(datetime.now())+'.csv'
# delimiter = ','
# level = logging.INFO
# custom_additional_levels = ['logs_c', 'logs_d']
...
class TestCompanionComputer(CompanionComputer):
    def __init__(self, sitlType, sitlPort):
        ...
        #self.radar = nanoRadar.NanoRadar()
        ...
        self.lat = 0 #Variables to store GPS data 
        self.lon = 0
        self.alt = 0
    ...
    def init(self):
        ...
        #self.get_mission_coordinates()some 
        time.sleep(2)
        ...
        self.tiRadar = tiRadar.TiRadar('util/xwr18xx_profile_2023_03_03T10_53_21_055_peak_grouping.cfg')
        #self.tiRadar = tiRadar.TiRadar('util/config_for_flashing.cfg')
        self.CLIport, self.Dataport = self.tiRadar.serialConfig(self.tiRadar.configFileName)
        self.configParameters = self.tiRadar.parseConfigFile(self.tiRadar.configFileName)
        # time.sleep(1)
        # self.tiRadar.stop_threads = True
        # self.tiRadar.kill()
        # self.CLIport, self.Dataport = self.tiRadar.serialConfig(self.tiRadar.configFileName)
        # self.configParameters = self.tiRadar.parseConfigFile(self.tiRadar.configFileName)

        try:
            thr2 = threading.Thread(target=self.tiRadar.run)
            thr2.start()
    ...
    def Obtain_Current_Waypoint_recent(self):
        while True:
            try:
                ...
                #print('currentwp is', self.i_current_waypoint)
            except KeyboardInterrupt:
                print("Exception in obtain_Current_Waypoint")

    def Custom_break_mode(self):
        try:
            print("Custom Break Mode")
            ...
            if self.currentMode == "GUIDED":
                self.mavlinkInterface.mavConnection.mav.mission_item_send(
                    self.mavlinkInterface.mavConnection.target_system,
                    self.mavlinkInterface.mavConnection.target_component,
                    0, 0,
                    mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT,
                    mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
                    0, 0, 0, 0, self.lat, self.lon, self.alt
                )
                
                # Wait for command to be acknowledged
                ack_msg = None
                while ack_msg is None or ack_msg.command != mavutil.mavlink.MAV_CMD_NAV_WAYPOINT:
                    ack_msg = self.mavlinkInterface.mavConnection.recv_match(type='COMMAND_ACK', blocking=True)
                    time.sleep(0.1)
                print("Breaked Successfully!")
        except Exception as e:
            print("Error at Custom Brake Mode ",e)
        ...
        while True:
            try:
                ...
                if msg is not None:
                    ...
                    self.alt = msg.alt/1000.0 #Altitude in mm, convert to m
                    break
            except:
...
#whenever in auto reset all the markers so that planning can be started again.
    ...
    def update_vars(self):
        ...
        self.coordinate_transform.terrainAlt=self.terrainAlt
        # tempx=self.tiRadar.x
        # tempy=self.tiRadar.y
        # tempz=self.tiRadar.z
        # tempvel=self.tiRadar.vel
        # self.front_sensor.Y = np.array(tempx)
        # self.front_sensor.X = np.array(tempy)
        # self.front_sensor.Z = np.array(tempz)
        # self.front_sensor.vel = np.array(tempvel)
        # for i in (self.tiRadar.x):
        #     if i>0 and i<2:
        #         self.front_sensor.Y = np.array(self.tiRadar.x)

        # for i in (self.tiRadar.y):
        #     if i>0 and i<2:
        #         self.front_sensor.X = np.array(self.tiRadar.y)
        # for i in (self.tiRadar.z):
        #     if i>0 and i<2:
        #         self.front_sensor.Z = np.array(self.tiRadar.z)
        # self.front_sensor.vel = np.array(self.tiRadar.vel)
        temp_x=self.tiRadar.y
        temp_y=self.tiRadar.x
        temp_z=self.tiRadar.z
        temp_vel=self.tiRadar.vel
        if(time.time()-self.tiRadar._read_time<0.3):
            if(len(temp_x) != 0 and len(temp_y) != 0 and len(temp_z) != 0 and len(temp_vel) != 0):
                if (np.array(temp_x)).max() < 40 and (np.array(temp_y)).max() < 40 and (np.array(temp_z)).max() < 40 and (np.array(temp_vel)).max() < 10:
                        self.front_sensor.X = np.array(temp_x)
                        self.front_sensor.Z = np.array(temp_z)
                        self.front_sensor.Y = np.array(temp_y)
                        self.front_sensor.vel=np.array(temp_vel)
                else:
                    print('some issue in radar values')
            else:
                pass
                #print('length of one of the arrays is zero') #sometimes one of the arrays is empty.
                
        else:
            print('no readings freshly')
            self.front_sensor.X = np.array([])
            self.front_sensor.Z = np.array([])
            self.front_sensor.Y = np.array([])
            self.front_sensor.vel=np.array([])

        # self.front_sensor.X = np.unique(np.clip((self.front_sensor.X), 0, 40))
        # self.front_sensor.Y = np.unique(np.clip((self.front_sensor.Y), 0, 40))
        # self.front_sensor.Z = np.unique(np.clip((self.front_sensor.Y), 0, 40))
        #print('front_sensor is',self.front_sensor.X)
        #print(len(self.front_sensor.X))
        time.sleep(0.05)
        
        # print("x: ",self.front_sensor.X)
        #print("y: ",self.front_sensor.Y)
        # print("z: ",self.front_sensor.Z)
        # time.sleep(1)
        try:
            self.log_data()
        except:
            print('some error in logging')
        #time.sleep(0.03)
        if len(self.front_sensor.X) != len(self.front_sensor.Y):
    ...
    #def printing(self):
        # while(True):
        #     try:
        #         fx = open("/home/pi/GA_IIT_Bombay/ga_companion_computer_imx/util/stupidx.txt","r")
        #         fy = open("/home/pi/GA_IIT_Bombay/ga_companion_computer_imx/util/stupidy.txt","r")
        #         fz = open("/home/pi/GA_IIT_Bombay/ga_companion_computer_imx/util/stupidz.txt","r")
        #         self.stupidx = fx.readline().strip()
        #         self.stupidy = fy.readline().strip()
        #         self.stupidz = fz.readline().strip()
        #         test_var_x = self.stupidx[1:len(self.stupidx)-1].split()
        #         test_var_y = self.stupidx[1:len(self.stupidy)-1].split()
        #         test_var_z = self.stupidx[1:len(self.stupidz)-1].split()
        #         #print(self.py)
        #         # self.front_sensor.Y = [float(elem[:len(elem)-1]) for elem in test_var_x]#Interchanging X and Y NED, I need to handle minus
        #         # self.front_sensor.X = [float(elem[:len(elem)-1]) for elem in test_var_y]
        #         # self.front_sensor.Z = [float(elem[:len(elem)-1]) for elem in test_var_z]
    ...
    def create_csv_file(self):
        ...
        self.csv_file = open(str(self.log_time1)+'.csv','w')
    ...
    def log_data(self):
        ...
        time.sleep(0.05)
        roll_pitch_yaw_string = str(round(self.roll,4))+','+str(round(self.pitch,4))+','+str(round(self.yaw,4))+"{}"
        terrain_alt_str = str(round(self.terrainAlt,2))+"{}"
        selfpx_and_selfpy_and_selfpz = str(round(self.px,2))+','+str(round(self.py,2))+','+str(round(self.pz,2))+'{}'
        ...
        try:
            frontSensorX = ','.join([str(round(elem,2)) for elem in self.front_sensor.X]) + '{}'
            frontSensorY = ','.join([str(round(elem,2)) for elem in self.front_sensor.Y])+"{}"
            frontSensorZ = ','.join([str(round(elem,2)) for elem in self.front_sensor.Z])+'{}'
            radvel = ','.join([str(round(elem,2)) for elem in self.front_sensor.vel])+'{}'
        except:
        
            print('error because x' ,frontSensorX)
            print('y',frontSensorY)
            print('z',frontSensorZ)
        # frontSensorX = self.stupidx
        # frontSensorY = self.stupidy
        # frontSensorZ = self.stupidz
        vx_vy_vz = str(round(self.vx,2))+','+str(round(self.vy,2))+'{}'
        #radvel = str(np.round(self.front_sensor.vel,2))+'{}'
        number=str(self.front_sensor.X.shape[0])+'{}'
        #final_data = str(time.time()) + frontSensorX + frontSensorY + frontSensorZ+ vx_vy_vz+'\n'
        #final_data = str(time.time()) + radvel + '\n'
        final_data = "Time," + str(time.time())+'{}' + str(self.log_time1) + '{}' + roll_pitch_yaw_string + terrain_alt_str + selfpx_and_selfpy_and_selfpz +vx_vy_vz+number+'{}' + frontSensorX +'{}' + frontSensorY +'{}' + frontSensorZ +'{}' + radvel + str(self.braked) + self.currentMode + '\n'
        # csvLogger.logs_c([time.time(), roll_pitch_yaw_string, terrain_alt_str, selfpx_and_selfpy_and_selfpz, vx_vy_vz, number])
        # csvLogger.logs_d([time.time(), frontSensorX, frontSensorY, frontSensorZ, radvel])
    ...
    def update_mission(self):    
        ...
        # print('printing',self.pitch)
        # time.sleep(0.1)
        # print('180th',self.lidar.raw_data[180])

        # print('270th',self.lidar.raw_data[270])
        print('inside update mission')
        self.front_sensor.handle_raw_data() #returns x and y coordinates of only masked points
        # write a logic to eliminate ground detection given a lidar array
        #print(np.array(list(zip(self.front_sensor.X,self.front_sensor.Y))))
        self.update_vars() #for transferign variable values across classesclear

        if self.currentMode == 'GUIDED':
            self.braked = 1
        else:
            self.braked = 0
##################General realtime data handling ##############################
        self.coordinate_transform.x = self.front_sensor.X
        self.coordinate_transform.y = self.front_sensor.Y  
        self.coordinate_transform.z = self.front_sensor.Z
        #print(self.front_sensor.X)
...
#deactivate for ground filtering

        #self.coordinate_transform.filter_ground()
...
#############################this is for grid mapping####################################
            # self.navigation_stack()
            # self.local_fetcher() #fetches local 2d occupancy grid        
            # self.local_mat_to_list()
            # self.np_mat_to_list=np.array(self.mat_list)
#####################This is for realtime sensing of data#############################
 
            self.np_mat_to_list=obs.T+np.array([self.px,self.py]) #array in inertial coordinates
            self.np_mat_to_list=np.array([[25,0]]) #change in straight line
             
            temp=self.i_waypoints[0] # means inertial home
            #print(temp[0],'temporary waypoint in', temp)
            #print('temp',temp,'current_waypoint', self.i_current_waypoint)
            #for handling edge waypoints #vector towards next waypoint          
            #print("current waypoint", self.i_current_waypoint)
            #print("px", self.px)
            #print('current waypoint is',self.i_current_waypoint)
            # if(self.i_current_waypoint==0):
            #     #print('current waypoint is', self.i_current_waypoint)
            #     pass
            # #print(self.i_current_waypoint,'i_current_waypotin')
            # print('terrain alt is ',self.terrainAlt)
            # time.sleep(1)
            #print('current waupoiyn is ',self.i_current_waypoint)
            # if(self.currentMode=='LOITER'):#and self.i_current_waypoint!=0 and self.terrainAlt>2 and self.i_current_waypoint!="TakeOff"):
            #     #if something is in heading direction, within desired angle and engaging distance, then engage brake/switch to guided
            #     #print('inside loop')
            ...
            #     #if(True):
            #         while(self.currentMode=='LOITER' and self.currentMode!='GUIDED'):
        ...
        # print('current waypoint is',self.i_current_waypoint)
        # a=self.i_waypoints[2]
        # print('second waypoint',a[1])
        if(self.currentMode=='AUTO' and self.i_current_waypoint!='TakeOff'):#and self.i_current_waypoint!=0 and self.terrainAlt>2 and self.i_current_waypoint!="TakeOff"):
                ...
                bool_temp,self.dist_temp=self.obstacle_in_direction(self.wpvec,self.np_mat_to_list,self.desired_angle,self.engaging_distance) # if any obstacle on the way
                #print('wpvec',self.wpvec,'bool',bool_temp,'distance',self.dist_temp)
                if(bool_temp and self.vec.mag2d(temp_wp_dist_vec)>abs(self.dist_temp)and (self.vx*self.vx+self.vy*self.vy)>2): #if obstacle needs to be avoided
                #print('vx,vy',self.vx,',',self.vy)
        ...
        #######################uncomment the below code for checking it in AUTO mode################################

        #print("(x,y)",self.front_sensor.X,self.front_sensor.Y)
        # self.coordinate_transform.x = self.front_sensor.X
        # self.coordinate_transform.y = self.front_sensor.Y  
        # self.coordinate_transform.z = self.front_sensor.Z
        # #print(self.front_sensor.X)
        ...
        #     self.np_mat_to_list=obs.T+np.array([self.px,self.py]) #array in inertial coordinates
        #     self.np_mat_to_list=np.array([[0,27]]) #change in straight line
        ...
        #     # time.sleep(1)
        #     #print('current waupoiyn is ',self.i_current_waypoint)
        #     if(self.currentMode=='AUTO'and self.i_current_waypoint!=0 and self.terrainAlt>2 and self.i_current_waypoint!="TakeOff"):
        #         #if something is in heading direction, within desired angle and engaging distance, then engage brake/switch to guided
        #         #print('inside loop')
        #         temp_wp_dist_vec=[self.i_current_waypoint[0]-self.px-temp[0],self.i_current_waypoint[1]-self.py-temp[1]] #used to find if the current vector to next waypoint i
    ...
    def handle_recieved_message(self):
        while True:    
            try:
                ...
                #print('received message is', recievedMsg)
