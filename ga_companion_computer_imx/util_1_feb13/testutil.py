...
#from util.Ti_UART import detObj
...
class TestCompanionComputer(CompanionComputer):
    def __init__(self, sitlType, sitlPort):
        ...
        self.lidar = prasLidar.PrasRPLidar_(4,-90,1,10)#scanMode, OffsetAngle, MinRange and MaxRange
        #Initialize Radar
        self.tiRadar = tiRadar.TiRadar('config.cfg')
        ...
        print('init')
        #Front sensor
        self.front_sensor = estimation.Sensor(1,math.pi/180,10,1,0)#maxRange minRange and Offset. I have already handled offset.
        #SITL
    ...
    def init(self):
        ...
        time.sleep(2)
        thr1 = threading.Thread(target=self.lidar.simple_express_scan)
        thr1.start()
        time.sleep(1)
...
#                 time.sleep(0.01)
    ...
    def update_mission(self):
        while True:
            ...
            self.coordinate_transform.x = self.front_sensor.X
            self.coordinate_transform.y = self.front_sensor.Y
            temp_x=self.front_sensor.X
            temp_y=self.front_sensor.Y
            print('front sensor data X', np.reshape(temp_x,[len(temp_x),1]))
            print('front sensor data Y',np.reshape(temp_y,[len(temp_y),1]))
            ...
            #print('roll',self.roll*180/math.pi,' pitch',self.pitch*180/math.pi,' yaw',self.yaw*180/math.pi)
    ...
    def navigation_stack(self):
        ...
        #self.navigation_map.convert_rel_obstacle_to_inertial(self.navigation_map.grid(self.coordinate_transform.obstacle_vector_inertial.T))
         #updates map in world frame
            ...
            print('map is ',self.navigation_map.map)
            for dummat in self.navigation_map.map:    
                ...
                print('self.matmap is',self.matmap)
